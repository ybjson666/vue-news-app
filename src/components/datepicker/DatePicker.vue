...
<template>
    <div class="select-control">
        ...
        <Picker :visible="visible" 
                ...
                @cancelEventInner="cancelEvent"
                @confirmEventInner="confirmEvent"
                >
        </Picker>
    </div>
</template>
<script>
    import Picker from './ItemWraps'
    export default {
        name: 'datePicker',
        data () {
            return {
        ...
        props: {
            ...
            confirmEventOuter:Function,
            cancelEventOuter:Function
        },
        components: {Picker},
        methods: {
            _change(value, index){
            ...
            _setDate(){
                ...
                //无起始和结束时间，显示前后10年
                if (isNaN(min)) {
                    yearMin = cur.getFullYear() - 30;
                } else {
                    yearMin = min.getFullYear();
                }
                if (isNaN(max)) {
                    yearMax = cur.getFullYear() + 30;
                } else {
            ...
            cancelEvent(){
                this.$emit('cancelEventOuter');
            },
            confirmEvent(value){
                this.$emit('confirmEventOuter',value);
            }
        },
        computed: {
            valueNew: {
