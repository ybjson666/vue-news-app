...
<template>
  <div class="pickers-item" @touchstart="_onTouchStart" @touchmove.prevent="_onTouchMove" @touchend="_onTouchEnd"
       @touchcancel="_onTouchEnd">
    <div class="picker-mask" :style="pickerMask"></div>
    <ul class="picker-li" :style="transformStyle">
      <li v-for="(item,index) in data" v-text="item" :key="index"></li>
    </ul>
  </div>
</template>
    ...
    methods: {
      ...
      _setIndex(index, bool) {
        ...
        if (this.data.length > 0) {
          ...
          //回调
          const value = this.data[index]&&this.data[index].value || this.data[index];
